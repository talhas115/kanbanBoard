...
public class TaskServiceTests
{
    ...
    public async Task AddWorkLogAsync_ValidHours_CreatesWorkLog()
    {
        ...
        var task = new Models.Task { Id = Guid.NewGuid(), Title = "Test", Status = "Backlog", Order = 0, CreatedById = user.Id };
    ...
    public async Task AssignTaskAsync_NewAssignee_CreatesHistory()
    {
        ...
        var task = new Models.Task { Id = Guid.NewGuid(), Title = "Test", Status = "Backlog", Order = 0, CreatedById = user1.Id, AssigneeId = user1.Id };
    ...
    public async Task AssignTaskAsync_SameAssignee_NoHistory()
    {
        ...
        var task = new Models.Task { Id = Guid.NewGuid(), Title = "Test", Status = "Backlog", Order = 0, CreatedById = user.Id, AssigneeId = user.Id };
        _context.Users.Add(user);
        _context.Tasks.Add(task);
        await _context.SaveChangesAsync();
    ...
    {
        var user = new User { Id = Guid.NewGuid(), Email = "user@example.com", PasswordHash = "hash" };
        var task1 = new Models.Task { Id = Guid.NewGuid(), Title = "Task 1", Status = "Backlog", Order = 0, CreatedById = user.Id };
        var task2 = new Models.Task { Id = Guid.NewGuid(), Title = "Task 2", Status = "Todo", Order = 0, CreatedById = user.Id };
        _context.Users.Add(user);
